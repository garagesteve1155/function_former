...
def llm(prompt: str, **gen_kwargs) -> str:
    ...
    def build_messages(pairs):
            ...
            # Keep the exact same note text; ONLY append it to the end of the prompt when FW mode is on.
            fw_note = ""
            ...
            # Base content stays the same as before; we still include the outline when present.
            if FULL_OUTLINE:
                base = f"(REFERENCE OUTLINE for entire script, always consider this when answering):\n{FULL_OUTLINE}\n\n{prompt}"
            else:
                base = prompt

            # ✅ The only FW-mode change: concatenate the note at the very end of the SAME message.
